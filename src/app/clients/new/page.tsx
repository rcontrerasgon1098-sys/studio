...
export default function NewClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Cliente Guardado", description: "El cliente ha sido registrado con éxito." });
      ...
      <header className="bg-white border-b sticky top-0 z-40 shadow-sm">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-lg md:text-xl text-primary">Agregar Nuevo Cliente</h1>
          </div>
          <Button onClick={handleSubmit} disabled={loading} className="bg-primary hover:bg-primary/90 h-10 px-6">
            <Save className="h-4 w-4 mr-2" /> {loading ? "Guardando..." : "Guardar Cliente"}
          </Button>
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="shadow-md border-none bg-white">
            <CardHeader className="bg-secondary/20 p-6 rounded-t-lg border-b">
              ...
              <CardDescription>Rellene los datos fiscales y de contacto del cliente.</CardDescription>
            </CardHeader>
