...
import { 
  ...
  Eye, Download, Menu, Users, BarChart3, TrendingUp 
} from "lucide-react";
...
import { collection, collectionGroup, query, where, orderBy, doc, getDoc } from "firebase/firestore";
import { 
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-primary tracking-tight">
              {isAdmin ? "Panel de Administración" : "Panel Técnico"}
            </h1>
            <p className="text-muted-foreground text-lg font-medium">
              {isAdmin ? "Vista Global Operativa" : `Bienvenido, ${user?.email?.split('@')[0]}`}
            </p>
          </div>
          {!isAdmin && (
            <Link href="/work-orders/new">
